...
data_df = pd.merge(data_df, pos_feature_df, on=['id'])
...
feat_df = data_df[
    list(pos_feature_df.columns) +
    list(matteo_features_df) +
    list(sent_word_features_df) +
    list(form_inform_target) +
    list(form_inform_post)
]
